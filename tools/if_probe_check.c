...
int main(void){
    uint32_t grp = dart_plat_parse_ip("239.255.0.7");
    uint32_t ifs[16];
    int n, i;
    dart_plat_startup();
    show("raw route_src(group):",   dart_plat_route_src(grp, 7400));
    show("route_src(192.0.2.1):",   dart_plat_route_src(dart_plat_ipv4(192,0,2,1), 7400));
    show("mcast_if_for(group) =>",  dart_discovery_mcast_if_for(grp, 7400));
    n = dart_plat_local_ipv4s(ifs, 16);
