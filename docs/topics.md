...
A topic is created at runtime and returned as an opaque `RambleTopic *`. Its identity across
peers is its name, so use the same name on every node. Locally a topic is its index in
creation order, which is `RambleMsg.topic_index` on delivery. Peers match on the name no
matter how each ordered its topics. `opts.max_topics` (default 8) bounds how many you can
create.

```c
RambleTopic *t = ramble_node_create_topic(n, "pose", RAMBLE_PUBSUB, schema,
                   &(RambleTopicOpts){ .qos = { .reliability = RAMBLE_RELIABLE, .keep_last = 16 } });
```

Roles (`RambleRole`) are `RAMBLE_PUBSUB`, `RAMBLE_PUB_ONLY`, `RAMBLE_SUB_ONLY` and
`RAMBLE_INACTIVE` (declared but off). `ramble_topic_set_role` flips the role at runtime and re advertises at once. Peers
rematch with no round trip.

The schema is optional. With one, a peer whose schema cannot read yours is refused with
`RAMBLE_E_SCHEMA_MISMATCH`. docs/stdtypes.md covers the schema DSL and the standard types.

## QoS

| field | meaning |
|---|---|
| `reliability` | `RAMBLE_BEST_EFFORT` or `RAMBLE_RELIABLE` |
...
`RAMBLE_E_QOS_INCOMPATIBLE` and forms on its own if the publisher later upgrades.
...
`ramble_topic_send(t, ramble_bytes(data, len))` commits the message to history and sends it
...
build has `RAMBLE_NO_SHM`. A message that fits one datagram always goes inline.
...
proceeds and fires `RAMBLE_E_UNMATCHED_SEND`.

For a GUI, disable the wait and check `ramble_topic_ready(t)` instead. It uses the same
predicate. `ramble_topic_pending_count` is the raw count of unresolved candidates.
`ramble_node_settle(n, timeout_ms)` waits for the whole node: open, create topics, settle,
publish.
...
`ramble_topic_retire(t)` says this node is done with this incarnation of the name. The
...
what it needed, fires one `RAMBLE_MSG_LOST` with the first seqno and the count.
spec/transport.md explains how a lapped reader rejoins.

## Counters

`ramble_topic_counts` returns messages and bytes sent and delivered on a topic. The same
numbers are in the `@ramble/meta` snapshot.
