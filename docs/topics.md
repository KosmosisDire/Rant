...
A topic is created at runtime and returned as an opaque `DartTopic *`. Its identity across
peers is its name, so use the same name on every node. Locally a topic is its index in
creation order, which is `DartMsg.topic_index` on delivery. Peers match on the name no
matter how each ordered its topics. `opts.max_topics` (default 8) bounds how many you can
create.

```c
DartTopic *t = dart_node_create_topic(n, "pose", DART_PUBSUB, schema,
                   &(DartTopicOpts){ .qos = { .reliability = DART_RELIABLE, .keep_last = 16 } });
```

Roles (`DartRole`) are `DART_PUBSUB`, `DART_PUB_ONLY`, `DART_SUB_ONLY` and
`DART_INACTIVE` (declared but off). `dart_topic_set_role` flips the role at runtime and re advertises at once. Peers
rematch with no round trip.

The schema is optional. With one, a peer whose schema cannot read yours is refused with
`DART_E_SCHEMA_MISMATCH`. docs/stdtypes.md covers the schema DSL and the standard types.

## QoS

| field | meaning |
|---|---|
| `reliability` | `DART_BEST_EFFORT` or `DART_RELIABLE` |
...
`DART_E_QOS_INCOMPATIBLE` and forms on its own if the publisher later upgrades.
...
`dart_topic_send(t, dart_bytes(data, len))` commits the message to history and sends it
...
build has `DART_NO_SHM`. A message that fits one datagram always goes inline.
...
proceeds and fires `DART_E_UNMATCHED_SEND`.

For a GUI, disable the wait and check `dart_topic_ready(t)` instead. It uses the same
predicate. `dart_topic_pending_count` is the raw count of unresolved candidates.
`dart_node_settle(n, timeout_ms)` waits for the whole node: open, create topics, settle,
publish.
...
`dart_topic_retire(t)` says this node is done with this incarnation of the name. The
...
what it needed, fires one `DART_MSG_LOST` with the first seqno and the count.
spec/transport.md explains how a lapped reader rejoins.

## Counters

`dart_topic_counts` returns messages and bytes sent and delivered on a topic. The same
numbers are in the `@dart/meta` snapshot.
