...
Every `ramble_node_*` and `ramble_topic_*` call is thread safe. A node level lock serializes
them and is never held across a blocking wait.

Drive a node one of two ways:

- Call `ramble_node_poll` from your own loop, from one thread or several. A send on another
  thread wakes a blocked poll.
- Call `ramble_node_start`. A background service thread owns the loop, `ramble_node_poll` then
  returns `RAMBLE_ERR_STATE`, and callbacks fire on that thread, never two at once per node.
  `ramble_node_stop` joins it. `ramble_node_close` implies stop.

Inside `on_message` or `on_event` you may call `ramble_topic_send` and read only queries.
Poll, create topic, set role, drain, start, stop and close are refused with
`RAMBLE_ERR_STATE` (or NULL or 0). Nothing is corrupted.
...
evict, KEEP_LAST proceeds and a `RAMBLE_E_EVICTED_UNSENT` error fires.
`ramble_node_evicted_unsent` counts them.

Reflection views (the names, addresses and schemas the `ramble_node_*_next` walks return)
are bracketed with `ramble_node_lock` and `ramble_node_unlock` when a poller runs on another
thread.

## Consumer queues

By default callbacks run on whichever thread polls, so a heavy handler lags the whole
node. A topic becomes queued on its first `ramble_topic_take` or `ramble_topic_dispatch`, or
...
- `ramble_topic_take` pops a zero copy view valid until that topic's next take or dispatch.
- `ramble_topic_dispatch` runs `on_message` on the calling thread outside the node lock, so
  those callbacks may use the whole API.
- `ramble_node_dispatch` drains every queued topic. Call it from a Unity `Update()` or a UI
  frame.
- `ramble_topic_queue_stats` observes the queue.

The ring starts small and grows to `qos.queue_bytes` (0 = `RAMBLE_QUEUE_CAP`, 1 MB). One
bigger message still fits. At the cap the reliability QoS decides: best effort overwrites
the oldest and fires `RAMBLE_MSG_LOST`, reliable parks delivery so the publisher's send
...
`RAMBLE_NO_THREADS`. From inside a callback they cannot wait.

The queue's extra copy costs about 15% of peak goodput at small payloads and up to 35% at
1 MB. Measure with `ramble_test queuebench`.

## Timestamps

Every `RambleMsg` carries three clocks.
...
```c
ramble_topic_send(topic, frame, &(RambleSendOpts){ .capture_us = exposed_at });
ramble_topic_send(topic, frame, NULL);        /* no capture time, no extra bytes */
```
...
forwards the commit stamp as `written_us` on `RambleRequest`, `RambleResponse`,
`RambleProgress` and `RambleVariableUpdate`, and carries no capture time: it describes
sensor data on a topic, not a call or a write.

## Errors and events

Everything that goes wrong arrives the same way: a `RambleEvent` with kind `RAMBLE_ERROR` and
an `.error` code (`RAMBLE_E_*`) naming the fault. `if (ev->kind == RAMBLE_ERROR)` is the one
"did something break" test, and `ramble_event_str` prints the full text.

The four other kinds are `RAMBLE_PEER_UP`, `RAMBLE_PEER_DOWN`, `RAMBLE_PEER_INTEREST` (peer
lifecycle) and `RAMBLE_MSG_LOST` (best effort loss, expected, not an error).
...
`.topic_name`. `ramble_last_error(node)` returns the last error by value. Pass `NULL` for
the process global slot that records why `ramble_node_open` failed. `RambleResult` (the
negative `RAMBLE_ERR_*` returned by `ramble_topic_send` and friends) is separate. Error
reporting costs nothing on the data path, and `RAMBLE_NO_DIAG` strips the text.

## Built in logs and meta

Every node hosts three shared reliable log topics: `@ramble/log/error`, `@ramble/log/warn` and
`@ramble/log/info`. `ramble_node_log(n, level, fmt, ...)` publishes a line, also from inside
callbacks. To read other nodes' lines, take your own handle with `ramble_node_log_topic`,
widen its role to `RAMBLE_PUBSUB`, and read it like any topic. A node never receives its
...
Internal `RAMBLE_ERROR` events are mirrored to the error level unless
`opts.disable_error_logs` is set.

Every node also hosts the `@ramble/meta` function, which answers a snapshot of its peers,
topics and counters for tools like the explorer. `opts.disable_meta` strips it.
`ramble_topic_counts` reads the per topic message and byte counters directly. Builtins are
