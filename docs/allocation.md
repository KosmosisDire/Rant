...
Sized by `max_peers` and `n_channels`. Independent of message size. Small and fixed
...
| buffer | count | size |
|--------|-------|------|
| writer history | n_channels x keep_last | max_message_bytes |
| reader reassembly | n_channels x max_peers | max_message_bytes |
| reader fragment bitmap | n_channels x max_peers | ceil(maxfrags / 8) |
...
`meta_cap` is the frag prefix plus the largest interest list the channels can
...
| slot binding | n_channels x keepmax x 2 |
| reader segment ids | rmax x 8 |
| reader pool handles | rmax x state_bytes |

`keepmax` is the largest keep_last across channels. `rmax` is
...
| DART_META_MAX_IDS | 256 | sizes the alias table |
