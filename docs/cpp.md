...
`dist/dart.hpp` is a header only C++17 wrapper over the C library, and the one file a
...
Exactly one translation unit defines `DART_IMPLEMENTATION` before the include. That unit
...
the C declarations inside `dart::detail` and nothing at global scope.
...
dart::Node node("robot1",
    [](const dart::MessageView& m){ /* every delivery */ },
    [](const dart::Event& e){ std::fprintf(stderr, "%s\n", e.to_string().c_str()); },
    { .domain = 7 });
...
`DART_SCHEMA(T, fields...)` goes at global scope after the struct, listing up to 64
...
those, `dart::String<N>` for a capped string, nested reflected structs, and the variable
...
Any wire type used directly as a handle's type is a bare schema with no `DART_SCHEMA`:
...
`DART_ENUM(E, options...)` registers an `enum class` so a member ships as a named
...
`dart::String<N>` is the capped string slot. `assign()` refuses an over capacity value and
...
but never as an array element. `dart::now()` and `dart::new_uuid()` are the two values
that need the platform.

Name your own type the same way: `DART_STD_STRUCT(T, fields...)` reflects the members and
names the type, and `DART_STD_ALIAS(T, R)` names a type that copies as `R`.
...
Pass `dart::reflect_from_mesh` where a constructor takes a schema pointer and the handle
...
overloads that truncate at `DART_LOG_MAX`. `on_log(level, handler)` widens the node's own
...
`meta_request(peer, handler, sections)` sends a directed `@dart/meta` call and decodes the
