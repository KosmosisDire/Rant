...
#ifndef RANT_TRANSPORT_INTERNAL_H
#define RANT_TRANSPORT_INTERNAL_H
...
#define RANT_HB     2
#define RANT_NACK 3
#define RANT_MSG_MASK 0x07u
#define RANT_F_SINGLE 0x08u       /* DATA: single fragment, frag, count and len omitted */
#define RANT_F_UNPOS    0x10u     /* NACK: the reader has delivered nothing yet */
#ifdef RANT_SHM
#define RANT_F_SHM      0x20u     /* DATA: the body is a descriptor, no payload */
#endif

#define RANT_NACK_WINDOW 32u      /* seqnos covered by one ACKNACK bitmap */
#define RANT__NIL 0xFFFFFFFFu

#ifndef RANT_HB_SWEEP_US
#define RANT_HB_SWEEP_US 25000u /* the timer sweep covers every lane this often */
#endif

#define RANT__NO_DEADLINE ((uint64_t)-1)    /* next_deadline_us: nothing armed */

/* per (peer, index) detail verdict bits, the peer_astate maps */
#define RANT__AST_DETAILED 0x01u    /* details received and judged, else pending */
#define RANT__AST_NAME_OK    0x02u  /* identity and name verified: peer_index[a] is our topic */
#define RANT__AST_READ_OK    0x04u  /* schema gate passed on the read side, their pub to our sub */
#define RANT__AST_WRITE_OK 0x08u    /* schema gate passed on the write side, their sub to our pub */
...
#ifdef RANT_SHM
#ifndef RANT_SHM_MAX_RETRY
#define RANT_SHM_MAX_RETRY 8u     /* give up on an unresolvable descriptor after this many */
#endif
#endif

#define RANT_QOS_DEF_KEEP_LAST       1u
#define RANT_QOS_DEF_KEEP_LAST_REL 10u     /* reliable: room for repair before overwrite */
#define RANT_QOS_DEF_HEARTBEAT_US 250000u     /* 250 ms idle writer heartbeat */
#define RANT_QOS_DEF_REPAIR_US      50000u    /* 50 ms reader repair request delay */
#ifndef RANT_RTO_MIN_US
#define RANT_RTO_MIN_US     2000u  /* the RTT timer floor, a poll wait cannot fire sooner */
#endif
#define RANT_RTO_GRAIN_US 1000u    /* RFC 6298 G: the deviation term never counts under one tick */
#ifndef RANT_HB_TAIL_US
#define RANT_HB_TAIL_US 20000u     /* the tail heartbeat before the peer's RTT is known */
#endif
...
#define RANT_OFFSET_INDEX        1u  /* u16, every submessage */
#define RANT_OFFSET_SEQNO        3u  /* DATA seqno, HB first, NACK base, SHM base (u64) */
#define RANT_OFFSET_PAYLOAD_LEN_SINGLE       11u  /* DATA single: u16 payload_len */
#define RANT_HEADER_DATA_SINGLE     13u  /* DATA single: header bytes */
#define RANT_OFFSET_FRAG        11u  /* DATA multi: u16 frag */
#define RANT_OFFSET_COUNT       13u  /* DATA multi: u16 count */
#define RANT_OFFSET_SAMPLE_LEN        15u  /* DATA multi: u32 sample_len */
#define RANT_OFFSET_PAYLOAD_LEN        19u  /* DATA multi: u16 payload_len */
#define RANT_HEADER_DATA_MULTI      21u  /* DATA multi: header bytes */
#define RANT_OFFSET_HB_LAST     11u  /* HB: u64 last */
#define RANT_OFFSET_HB_COUNT      19u  /* HB: u32 count */
#define RANT_HEADER_HB        23u
#define RANT_OFFSET_NACK_NBITS    11u  /* NACK: u16 nbits */
#define RANT_OFFSET_NACK_BITMAP     13u  /* NACK: u32 bitmap */
#define RANT_OFFSET_NACK_EPOCH    17u  /* NACK: u32 epoch */
#define RANT_HEADER_NACK      21u
#define RANT_OFFSET_SHM_COUNT 11u    /* SHM-DATA: u16 count */
#define RANT_OFFSET_SHM_DESC    13u  /* SHM-DATA: the descriptor */
#ifdef RANT_SHM
#define RANT_SHM_DATA_BYTES (RANT_OFFSET_SHM_DESC + RANT_SHM_DESC_BYTES)
#endif
...
#define RANT__DEST_ALL    0xFFFFFFFFu
#define RANT__DEST_NONE 0xFFFFFFFEu
...
typedef struct {
    ...
    uint32_t dest_slot;  /* RANT__DEST_ALL, RANT__DEST_NONE, or the destination peer slot */
    uint16_t count;      /* frag count */
    uint8_t  valid;
#ifdef RANT_SHM
    uint8_t  shm;        /* 1 = the bytes live in shm_buf and desc is set */
    uint8_t  desc[RANT_SHM_DESC_BYTES];/* sent to SHM peers as one SHM-DATA */
#endif
} i_RantWriterSample;
...
typedef struct {        /* reader side, per (topic, peer) */
    ...
    uint64_t ack_due_us;
    i_RantAssembly cur;       /* the head sample */
    i_RantAssembly next;      /* one sample held ahead of the head. See spec/transport.md */
...
typedef struct {
    ...
    /* writer */
    i_RantWriterSample *history;         /* [depth] ring */
...
struct RantTransportState {
    ...
    uint8_t    **peer_astate;     /* [max_peers] RANT__AST_* bits per entry */
    ...
    i_RantTopic    *topics;     /* [n_topics] */
    /* the lane record pool: one hook allocation grown by doubling, records per real match */
    i_RantLane    *lanes;       /* [lane_cap] */
    uint32_t     lane_cap;
    uint32_t     lane_free;   /* free list head, RANT__NIL when empty */
...
typedef enum { RANT_ORDER_OLD, RANT_ORDER_GAP, RANT_ORDER_ADOPTED, RANT_ORDER_INORDER } i_RantReaderOrder;

static inline void i_rant_bit_set(uint8_t*bitmap,uint32_t i){bitmap[i>>3]|=(uint8_t)(1u<<(i&7));}
static inline void i_rant_bit_clr(uint8_t*bitmap,uint32_t i){bitmap[i>>3]&=(uint8_t)~(1u<<(i&7));}
static inline int  i_rant_bit_get(const uint8_t*bitmap,uint32_t i){return (bitmap[i>>3]>>(i&7))&1;}
/* Does this slot occupy an announce entry. An undefined or retired slot rides as a hole run. */
static inline int i_rant_topic_announced(const i_RantTopic *t){
    return t->name_len != 0 && !t->retired;
}
#ifdef RANT_SHM
/* where a sample's bytes live: the external chunk for SHM samples, else our buf */
static inline const uint8_t *i_rant_sample_buf(const i_RantWriterSample *s){ return s->shm ? s->shm_buf : s->buf; }
#else
static inline const uint8_t *i_rant_sample_buf(const i_RantWriterSample *s){ return s->buf; }
#endif
...
static inline uint32_t i_rant_topic_ts_bytes(const i_RantTopic *topic){
    return topic->qos.no_timestamp ? 0u : RANT_TIMESTAMP_BYTES;
}
...
static inline uint32_t i_rant_topic_stamp_bytes(const i_RantTopic *topic, uint64_t capture_us){
    uint32_t ts = i_rant_topic_ts_bytes(topic);
    return (ts && capture_us) ? ts + RANT_CAPTURE_BYTES : ts;
}
...
static inline int i_rant_topic_retains_history(const i_RantTopic *topic){
    return topic->qos.reliability==RANT_RELIABLE && topic->qos.catch_up>0;
}
...
static inline int i_rant_topic_needs_sweep(const i_RantTopic *topic){
    return topic->qos.reliability==RANT_RELIABLE && (topic->matched_writers || topic->matched_readers);
}
...
size_t i_rant_wire_mk_data(uint8_t *o, uint16_t index, uint64_t seqno, i_RantWriterSample *s, uint16_t frag, const uint8_t *payload, uint16_t payload_len);
#ifdef RANT_SHM
size_t i_rant_wire_mk_shm(uint8_t *o, uint16_t index, uint64_t base, uint16_t count, const uint8_t *desc);
#endif
