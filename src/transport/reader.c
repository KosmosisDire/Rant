...
int rant_transport_subscriber_progress(RantTransportState *st, uint16_t topic_index, uint32_t peer,
    ...
    if (!topic) return 0;
    peer_slot = i_rant_peer_slot(st, peer);
    if (peer_slot < 0) return 0;
    r = i_rant_reader_proxy_at(st,topic_index,peer_slot);
    ...
    if (have){
        uint32_t i, c=0;
        for (i=0;i<r->cur.count;i++) if (i_rant_bit_get(r->cur.bitmap,i)) c++;
        *have = c;
    }
    return 1;
}
...
static void i_rant_reader_arm(i_RantTopic *topic, i_RantReaderProxy *r, int is_hb){
...
static void i_rant_reader_ack_now(RantTransportState *st, i_RantTopic *topic, i_RantReaderProxy *r,
    ...
    i_rant_lane_wake(st, topic_index, peer_slot);
}
...
static int i_rant_asm_missing(const i_RantAssembly *a, uint64_t s){
    return !(a->active && s >= a->base && s < a->base + a->count && i_rant_bit_get(a->bitmap, (uint32_t)(s - a->base)));
}
...
static void i_rant_reader_deliver(RantTransportState *st, uint16_t topic_index, uint32_t peer_slot){
    ...
    while (r->cur.active && r->cur.low == r->cur.count){
        int refused = st->cfg.on_message &&
                      st->cfg.on_message(st->cfg.user, topic_index, st->peer_ids[peer_slot],
                                         rant_bytes(r->cur.buf, r->cur.len)) != 0;
        r = i_rant_reader_proxy_at(st,topic_index,peer_slot);
        if (refused && reliable){
    ...
    if (delivered && reliable)              /* ack after delivery, the reader owns copy invariant */
        i_rant_reader_ack_now(st,topic,r,topic_index,peer_slot,1,0);
}
...
static i_RantReaderOrder i_rant_reader_order_arrival(RantTransportState *st, int topic_index, int peer_slot,
    ...
    if (base > r->deliver_upto){
        if (topic->qos.reliability==RANT_RELIABLE && r->started){     /* gap: arm a repair NACK */
            /* an armed lane keeps its deadline and is only re woken. A parked lane stays silent */
            if (!r->parked){
                if (r->ack_pending) i_rant_lane_wake(st,(uint16_t)topic_index,(uint32_t)peer_slot);
                else i_rant_reader_ack_now(st,topic,r,(uint16_t)topic_index,(uint32_t)peer_slot,0,0);
            }
        ...
        r->deliver_upto = base;
        return RANT_ORDER_ADOPTED;
    }
    return RANT_ORDER_INORDER;
}
...
void i_rant_reader_shm(RantTransportState *st, int topic_index, int peer_slot, const uint8_t *p, uint64_t now){
    ...
    i_RantReaderProxy *r=i_rant_reader_proxy_at(st,topic_index,peer_slot);
    int reliable = (topic->qos.reliability==RANT_RELIABLE);
    uint64_t base = i_rant_le_r64(p+RANT_OFFSET_SEQNO);
    uint16_t count = i_rant_le_r16(p+RANT_OFFSET_SHM_COUNT);
    const uint8_t *desc = p+RANT_OFFSET_SHM_DESC;
    if (!r || !r->used || count==0) return;
    if (r->parked){ topic->repair_stats.frags_ahead++; return; }   /* a held sample blocks */
    {   i_RantReaderOrder ord = i_rant_reader_order_arrival(st, topic_index, peer_slot, r, base, base+count-1);
        if (ord==RANT_ORDER_OLD || ord==RANT_ORDER_GAP) return;       /* old, dup, or gap armed */
    }
    ...
    if (r->rtt_probe && r->rtt_probe_seq >= base && r->rtt_probe_seq < base + count){
        i_rant_rtt_sample(st, (uint32_t)peer_slot, now > r->rtt_probe_us ? now - r->rtt_probe_us : 0u);
        r->rtt_probe = 0;
    }
    /* Advance and ack only if the node delivered. A failed resolve leaves the gap for the
       reliability layer, and after RANT_SHM_MAX_RETRY tries it is skipped loudly. */
    {   int ok = st->cfg.on_shm ?
                 st->cfg.on_shm(st->cfg.user, (uint16_t)topic_index, st->peer_ids[peer_slot], desc) : 0;
        r = i_rant_reader_proxy_at(st,topic_index,peer_slot);     /* the callback may re enter */
        if (ok > 0){
            r->shm_fail = 0; r->lapped = 0;
            r->deliver_upto = base + count;
            i_rant_reader_settle(r);
            if (reliable)                   /* ack after delivery */
                i_rant_reader_ack_now(st,topic,r,(uint16_t)topic_index,(uint32_t)peer_slot,1,0);
            i_rant_reader_deliver(st,(uint16_t)topic_index,(uint32_t)peer_slot);     /* held ahead */
            return;
        }
        if (ok < 0){                        /* refused downstream */
            ...
            if (i_rant_asm_fit(st, &r->cur, RANT_SHM_DESC_BYTES, 1u)){
                memcpy(r->cur.buf, desc, RANT_SHM_DESC_BYTES);
        ...
        if (reliable && ++r->shm_fail >= RANT_SHM_MAX_RETRY){
            ...
            i_rant_reader_ack_now(st,topic,r,(uint16_t)topic_index,(uint32_t)peer_slot,0,0);
        } else {
            r->deliver_upto = base + count;             /* best effort: no repair */
            i_rant_reader_settle(r);
            i_rant_lane_wake(st,(uint16_t)topic_index,(uint32_t)peer_slot);
        }
    }
}
#endif
...
void i_rant_reader_data(RantTransportState *st, int topic_index, int peer_slot, const uint8_t *p,
    ...
    if (p[0] & RANT_F_SINGLE){             /* single fragment: frag, count and len are implied */
        seqno=i_rant_le_r64(p+RANT_OFFSET_SEQNO); frag=0; count=1; payload_len=i_rant_le_r16(p+RANT_OFFSET_PAYLOAD_LEN_SINGLE); sample_len=payload_len; payload=p+RANT_HEADER_DATA_SINGLE;
    } else {
        seqno=i_rant_le_r64(p+RANT_OFFSET_SEQNO); frag=i_rant_le_r16(p+RANT_OFFSET_FRAG); count=i_rant_le_r16(p+RANT_OFFSET_COUNT);
        sample_len=i_rant_le_r32(p+RANT_OFFSET_SAMPLE_LEN); payload_len=i_rant_le_r16(p+RANT_OFFSET_PAYLOAD_LEN); payload=p+RANT_HEADER_DATA_MULTI;
    }
    base = seqno - frag;
    ...
    switch (i_rant_reader_order_arrival(st, topic_index, peer_slot, r, base, seqno)){
        case RANT_ORDER_OLD:       topic->repair_stats.frags_old++;   return;   /* already seen */
        case RANT_ORDER_GAP:     /* a future sample: hold one ahead */
            a = i_rant_reader_ahead(r, base);
            if (!a){ topic->repair_stats.frags_ahead++; return; }   /* hold taken */
            break;
        case RANT_ORDER_ADOPTED: topic->repair_stats.frags_ahead++; r->cur.active=0; a=&r->cur; break;
        case RANT_ORDER_INORDER:
    ...
    if (!i_rant_asm_fit(st, a, sample_len, count)){
        ...
        i_rant_reader_settle(r);
        if (reliable) i_rant_reader_ack_now(st,topic,r,(uint16_t)topic_index,(uint32_t)peer_slot,1,0);
        i_rant_reader_deliver(st,(uint16_t)topic_index,(uint32_t)peer_slot);
        return;
    }
    if (!a->active){
    ...
    new_frag = !i_rant_bit_get(a->bitmap,frag);
    if (new_frag){
        ...
        i_rant_bit_set(a->bitmap,frag);
        if (frag==a->low)                                    /* extended the contiguous front */
            while (a->low<count && i_rant_bit_get(a->bitmap,a->low)) a->low++;
    ...
    if (a->low == count) i_rant_reader_deliver(st,(uint16_t)topic_index,(uint32_t)peer_slot);
...
void i_rant_reader_hb(RantTransportState *st, int topic_index, int peer_slot, const uint8_t *p, uint64_t now){
    ...
    (void)now;
    if (!r || !r->used) return;
    if (topic->qos.reliability!=RANT_RELIABLE) return;
    ...
    r->hb_last=last;
    i_rant_reader_ack_now(st,topic,r,(uint16_t)topic_index,(uint32_t)peer_slot,1,1);
}
...
size_t i_rant_reader_emit(RantTransportState *st, int topic_index, int peer_slot, uint8_t *out, size_t cap, uint64_t now){
    ...
    uint16_t index = i_rant_wire_index_of(st, topic_index);
    int due, holes=0, repair=0, force;
    if (!r || !r->used || st->peer_dormant[peer_slot]) return 0;   /* unmatched or dormant */
    if (topic->qos.reliability!=RANT_RELIABLE) return 0;
    if (cap<RANT_HEADER_NACK) return 0;
    ...
    if (!r->started)   /* no position yet: F_UNPOS announces our epoch */
        return i_rant_wire_mk_nack(out,index,r->deliver_upto,0,0,r->epoch,(uint8_t)RANT_F_UNPOS);
    ...
    if (first_missing <= bound){                         /* a hole sits below something we heard */
        holes = 1;
        top = first_missing + RANT_NACK_WINDOW;            /* one window per ACKNACK */
        ...
        {   /* in flight dedup is valid only while assembling this message. With no head, and
            ...
            int have_probe = 0;
            for (s=from; s<top; s++){
                if (i_rant_asm_missing(&r->cur, s) && i_rant_asm_missing(&r->next, s)){
            ...
            if (bitmap){
                ...
                uint32_t rto = topic->qos.repair_delay_us ? topic->qos.repair_delay_us
                             : i_rant_rtt_rto(st, (uint32_t)peer_slot, RANT_QOS_DEF_REPAIR_US);
    ...
    if (holes){ r->ack_pending=1; r->ack_due_us=r->nack_retransmit_us; i_rant_transport_arm_deadline(st,r->ack_due_us); }

    /* only a repair request or a forced cumulative ack is worth a datagram */
    if (!repair && !force) return 0;
    return i_rant_wire_mk_nack(out,index,first_missing,nbits,bitmap,r->epoch,0);
}
...
uint32_t rant_transport_deliver_parked(RantTransportState *st, uint16_t topic_index, uint64_t now){
    ...
    while (li != RANT__NIL){
        ...
        if (st->lanes[li].r.used && st->lanes[li].r.parked){
            ...
            i_RantReaderProxy *r = &st->lanes[li].r;
            int accepted;
#ifdef RANT_SHM
            if (r->parked_shm){
                int ok = st->cfg.on_shm ?
                ...
                if (ok == 0){
                    ...
                    i_rant_reader_ack_now(st,topic,r,topic_index,peer_slot,0,0);
                    li = next; continue;
                }
                accepted = (ok > 0);
                if (accepted){
                    ...
                    i_rant_reader_settle(r);
                    if (topic->qos.reliability==RANT_RELIABLE)      /* ack after delivery */
                        i_rant_reader_ack_now(st,topic,r,topic_index,peer_slot,1,0);
                    i_rant_reader_deliver(st,topic_index,peer_slot);
                }
            } else
#endif
            {
