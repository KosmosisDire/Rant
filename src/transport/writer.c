...
static i_RantWriterSample *i_rant_sample_find(i_RantTopic *topic, uint64_t seqno){
    uint16_t depth = topic->qos.keep_last, k;
    uint16_t i = topic->history_head;
    for (k=0;k<depth;k++){
        i_RantWriterSample *s;
...
static void i_rant_writer_seal(RantTransportState *st, i_RantTopic *topic, size_t len,
    ...
    i_RantWriterSample *slot = &topic->history[topic->history_head];
    if (count==0) count=1;
...
static void i_rant_writer_lane_advance(i_RantTopic *topic, i_RantWriterProxy *w, uint32_t peer_slot){
    i_RantWriterSample *s;
    if (!topic->directed) return;
    while ((s = i_rant_sample_find(topic, w->acked_upto)) != NULL
           && s->dest_slot != RANT__DEST_ALL && s->dest_slot != peer_slot){
        w->acked_upto = s->base + s->count;
        w->skip_hb = 1;
    }
    if (w->sent_upto < w->acked_upto) w->sent_upto = w->acked_upto;
    while (w->sent_upto < topic->next_seqno
           && (s = i_rant_sample_find(topic, w->sent_upto)) != NULL
           && s->dest_slot != RANT__DEST_ALL && s->dest_slot != peer_slot
           && w->sent_upto == s->base)
...
static void i_rant_writer_commit(RantTransportState *st, uint16_t topic_index, size_t len,
    ...
    uint32_t li;
    i_rant_writer_seal(st, topic, len, dest_slot, &base, &count);
    for (li=topic->lane_head; li!=RANT__NIL; li=st->lanes[li].topic_next){
        ...
        if (dest_slot == RANT__DEST_ALL || l->peer_slot == dest_slot){ i_rant_lane_enqueue(st, li); continue; }
        i_rant_writer_lane_advance(topic, &l->w, l->peer_slot);
        if (reliable && l->w.reader_reliable && l->w.skip_hb) i_rant_lane_enqueue(st, li);
    }
}
...
static int i_rant_writer_store(RantTransportState *st, i_RantTopic *topic,
    ...
    uint32_t cap_b = (ts && capture_us) ? RANT_CAPTURE_BYTES : 0u;
    size_t len = (size_t)ts + cap_b + hdr.len + data.len;
    if (i_rant_writer_too_big(st, topic, len)) return RANT_ERR_TOO_BIG;
    {   i_RantWriterSample *slot = &topic->history[topic->history_head];
        ...
        if ((size_t)slot->cap < need){                    /* grow the slot to fit */
            ...
            if (!new_buf) return RANT_ERR_OOM;
    ...
    {   uint8_t *dst = topic->history[topic->history_head].buf;    /* gather: ts, hdr, payload */
        if (ts){
            ...
            w &= RANT_STAMP_MASK;
            if (cap_b) w |= RANT_STAMP_CAPTURE;
            i_rant_le_w64(dst, w);
            if (cap_b) i_rant_le_w64(dst + ts, capture_us);
        }
...
static int i_rant_writer_send(RantTransportState *st, uint16_t topic_index,
    ...
    if (i_rant_writer_too_big(st, topic,
            i_rant_topic_stamp_bytes(topic, capture_us) + hdr.len + data.len))
        return RANT_ERR_TOO_BIG;
    if (topic->role == RANT_SUB_ONLY || topic->role == RANT_INACTIVE) return RANT_ERR_ROLE;
    /* nobody subscribes and nothing durable to keep: skip the grow, the copy and the commit */
    if (topic->matched_writers == 0 && !i_rant_topic_retains_history(topic)) return RANT_OK;
    r = i_rant_writer_store(st, topic, hdr, data, capture_us, &len);
    if (r != RANT_OK) return r;
    i_rant_writer_commit(st, topic_index, len, dest_slot);
    return RANT_OK;
}
...
int rant_transport_send_to(RantTransportState *st, uint16_t topic_index, uint32_t to_peer,
    ...
    (void)now;
    return i_rant_writer_send(st, topic_index, hdr, data, capture_us,
                              peer_slot < 0 ? RANT__DEST_NONE : (uint32_t)peer_slot);
}
...
int rant_transport_send_shm(RantTransportState *st, uint16_t topic_index, RantBytes chunk,
    ...
    topic = i_rant_topic_at(st, topic_index, NULL);
    if (!topic) return RANT_ERR_NO_TOPIC;
    if (len > 65535u*(uint32_t)st->frag) return RANT_ERR_TOO_BIG;        /* the fragment count cap */
    if (topic->role == RANT_SUB_ONLY || topic->role == RANT_INACTIVE) return RANT_ERR_ROLE;
    slot = &topic->history[topic->history_head];
    slot->shm = 1;
    slot->shm_buf = chunk.data;
    memcpy(slot->desc, desc, RANT_SHM_DESC_BYTES);
    i_rant_writer_commit(st, topic_index, len, RANT__DEST_ALL);
    return RANT_OK;
}
#endif
...
static size_t i_rant_writer_hb(RantTransportState *st, i_RantTopic *topic, i_RantWriterProxy *w, uint16_t index,
    ...
    i_rant_transport_arm_deadline(st, w->hb_next_us);     /* wake to send the next idle HB */
    w->hb_count++;
    return i_rant_wire_mk_hb(out, index, first, topic->next_seqno-1, w->hb_count);
}
...
void i_rant_writer_nack(RantTransportState *st, int topic_index, int peer_slot, const uint8_t *p, uint64_t now){
    ...
    uint32_t epoch=i_rant_le_r32(p+RANT_OFFSET_NACK_EPOCH); uint8_t flags=p[0];
    if (!w || !w->used) return;
    if (w->reader_epoch != epoch){
        if (w->reader_epoch){
            ...
            uint64_t join = i_rant_topic_unicast_join_seqno(topic);
    ...
    if (w->rtt_probe && base >= w->rtt_probe_seq){
        i_rant_rtt_sample(st, (uint32_t)peer_slot, now > w->rtt_probe_us ? now - w->rtt_probe_us : 0u);
        w->rtt_probe = 0;
    }
    ...
    i_rant_writer_lane_advance(topic, w, (uint32_t)peer_slot);
    if (w->skip_hb) i_rant_lane_wake(st,(uint16_t)topic_index,(uint32_t)peer_slot);
    ...
    if (w->has_nack && base > w->nack_base){
        ...
        w->nack_bits = d < RANT_NACK_WINDOW ? w->nack_bits >> d : 0u;
        w->nack_base = base;
    ...
    if (nbits>0 && bitmap!=0){
        ...
        if (w->has_nack){                                           /* nack_base >= base here */
            ...
            w->nack_bits |= d < RANT_NACK_WINDOW ? bitmap >> d : 0u;
...
size_t i_rant_writer_emit(RantTransportState *st, int topic_index, int peer_slot, uint8_t *out, size_t cap, uint64_t now){
    ...
    if (reliable && w->has_nack){
        ...
        for (i=0;i<RANT_NACK_WINDOW;i++){
            if (w->nack_bits & (1u<<i)){
                ...
                s=i_rant_sample_find(topic,seqno);
                if (s && topic->directed && s->dest_slot != RANT__DEST_ALL
                ...
                if (s){
                    ...
                    if (st->peer_shm[peer_slot] && s->shm){   /* resend as one SHM-DATA */
                        uint32_t j;
                        if (cap < RANT_SHM_DATA_BYTES) return 0;
                        for (j=0;j<RANT_NACK_WINDOW;j++){
                            uint64_t sq=w->nack_base+j;
                            if (sq>=s->base && sq<s->base+s->count) w->nack_bits &= ~(1u<<j);
                        }
                        if (w->nack_bits==0) w->has_nack=0;
                        return i_rant_wire_mk_shm(out,index,s->base,s->count,s->desc);
                    }
#endif
                    {
                    ...
                    if (cap < (size_t)(s->count==1?RANT_HEADER_DATA_SINGLE:RANT_HEADER_DATA_MULTI)+(size_t)payload_len) return 0;
                    w->nack_bits &= ~(1u<<i);
                    if (w->nack_bits==0) w->has_nack=0;
                    topic->repair_stats.frags_sent++; topic->repair_stats.frags_resent++;
                    return i_rant_wire_mk_data(out,index,seqno,s,frag_index,i_rant_sample_buf(s)+offset,payload_len);
                    }
                } else {
                    ...
                    if (w->nack_bits==0) w->has_nack=0;
                    return i_rant_writer_hb(st,topic,w,index,out,cap,now);
                }
            }
        }
        w->has_nack=0;
    ...
    if (reliable && w->reader_reliable && w->skip_hb){
        size_t hb = i_rant_writer_hb(st,topic,w,index,out,cap,now);
    ...
    if (w->sent_upto < topic->next_seqno){
        ...
        i_RantWriterSample *s=i_rant_sample_find(topic,seqno);
        ...
        if (per_lane && w->rate_interval_us && (!s || seqno == s->base)){
            if (now < w->rate_next_us){ i_rant_transport_arm_deadline(st, w->rate_next_us); return 0; }
            { i_RantWriterSample *newest = i_rant_sample_find(topic, topic->next_seqno - 1);
            ...
            i_rant_transport_arm_deadline(st, w->rate_next_us);     /* the next tick fires on time */
        }
        if (s){
#ifdef RANT_SHM
            ...
            if (st->peer_shm[peer_slot] && s->shm){
                ...
                if (reliable && w->reader_reliable){
                    ...
                    if (w->sent_upto >= topic->next_seqno) i_rant_writer_arm_tail(st, w, (uint32_t)peer_slot, now);
                }
                return i_rant_wire_mk_shm(out,index,per_lane ? s->base - w->wire_skip : s->base,
                                          s->count,s->desc);
            }
#endif
            {
            ...
            if (reliable && w->reader_reliable){
                ...
                if (w->sent_upto >= topic->next_seqno)
                    i_rant_writer_arm_tail(st, w, (uint32_t)peer_slot, now);     /* tail HB */
            }
            return i_rant_wire_mk_data(out,index,per_lane ? seqno - w->wire_skip : seqno,
                                       s,frag_index,i_rant_sample_buf(s)+offset,payload_len);
            }
        } else {
