...
Rant with an OS ifdef. A runtime speaks only `i_rant_plat_*` and never touches a sockaddr.
...
called a naddr. The two are the same four bytes. `i_rant_plat_ip4_to_naddr` and
`i_rant_plat_naddr_to_ip4` move between them.
...
|---|---|---|
| `RANT_SHM` | Windows, Linux, macOS, the BSDs | file mappings, or shm_open and mmap. Older glibc wants `-lrt` |
| `RANT_THREADS` | the same plus ESP-IDF and any libc with pthread.h | thread, mutex, condvar and waker. Older toolchains want `-lpthread` |
| `RANT_PROC_STATS` | the same plus ESP-IDF | GetProcessTimes, getrusage or the ESP heap API |

When `RANT_PROC_STATS` is off the two stats functions are absent and every consumer
...
- Startup and cleanup. WSAStartup and timeBeginPeriod are refcounted by the OS per
  process, so matched calls pair safely from any thread and Rant keeps no counter. The 1 ms
  timer matters: the default 15.6 ms tick throttles ACK and repair rates.
  `RANT_NO_HIGHRES_TIMER` skips it.
...
- Receive. Windows fails an oversized datagram with WSAEMSGSIZE after filling the buffer,
  ...
  `i_rant_plat_suppress_connreset` stops an ICMP port unreachable from failing the next
...
- Interfaces. `i_rant_plat_local_ifaces` returns the up, non loopback IPv4 interfaces with
