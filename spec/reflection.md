...
How the node builds the views behind `dart_node_peers_next`, `dart_node_entities_next`
and `dart_node_mesh_next`. The API is in docs/reflection.md.
...
rebuilt lazily after a change, and `dart_node_mesh_epoch` bumps when a walk should run
...
There is no public subscriber side match count. `dart_topic_match_count` is publisher
...
name schema ranking (`dart_node_mesh_schema`) is gone: the provider rule plus writer side
