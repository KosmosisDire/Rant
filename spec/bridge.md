...
`bridge/` builds `dart_bridge`, a C++17 server where one WebSocket connection is one full
DART node, built on `dist/dart.hpp`. `bridge/PROTOCOL.md` is the wire spec (v11) and must
...
`DART_BRIDGE_WEBRTC`. `bridge/cmake/FindMbedTLS.cmake` shadows the finders the fetched
...
WebRTC down first, then the node. A background `dart_bridge.exe` blocks the link of a
