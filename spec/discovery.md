...
startup solicit, the first `DART_DISCOVERY_BLOB_RESEND` (3) announces after a change, and
...
| DROPPED | silent past the timeout. The slot and `local_id` are kept, `on_peer_down` fires with reason `DART_DISCOVERY_DROP`, and the node marks it dormant: out of flow control, proxies and delivery position kept, inbound still processed |
...
`DART_DISCOVERY_GONE`). A table full of ACTIVE peers refuses the newcomer
(`on_peer_refused`, `DART_E_PEER_REFUSED`) rather than evicting a live one.
...
host announce. Auto mode re enumerates every 3 s (`DART_DISCOVERY_IF_SCAN_US`), so a link
...
`dart_discovery_set_local_subnets`. A sans-IO caller that does not gets only the routed
...
`RELAY_ME` announce DIRECTLY re announces it: `dart_discovery_poll_relay` builds an
...
2. Peers bind one OBSERVED source per arrival channel (`obs_disc`, `obs_data`) from direct
   `RELAY_ME` announces, keyed by uuid, with `DartDiscoveryVia` saying which local socket.
   ...
   ranking and black hole every send to that peer. `dart_discovery_addr_of_id` (the
...
3. The NAT'd side must speak first, so relays also run introductions
   (`dart_discovery_poll_introduce`): each relay me peer is sent proxied announces of the
   ...
   repair sweep every `DART_DISCOVERY_INTRODUCE_SWEEP` = 10 announce intervals), never
