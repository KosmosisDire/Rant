...
init or reserves slots that `dart_topic_define` fills later, which is how
`dart_node_create_topic` works.
...
The list inlines while the whole datagram fits `DART_DGRAM_MAX` (about 250 topics at
...
`dart_node_peer_interest_next`, never `dart_meta_interest(v.meta)` directly. The
...
attrs byte (`DART_ATTR_*`: no timestamp, multi, forceable, cancellable, exclusive), its
...
different name peer fires `DART_E_NAME_COLLISION` and is refused), then the schema gate
...
but the cached non match made apply skip it forever. `dart_transport_topic_define` sends
...
`dart_node_peer_topic_name` and `dart_node_peer_topic_schema` return that node owned
...
become load bearing for matching. `@dart/meta` is that facility, kept out of the
matching path.
...
ceiling. A failed map allocation surfaces as `DART_E_INTEREST_OVERFLOW`. The maps bind a
...
refused with `DART_E_QOS_INCOMPATIBLE` and forms on its own if the publisher upgrades. On
...
A role flip via `dart_topic_set_role` re advertises at once. A (re)subscribe joins like a
...
`dart_topic_retire` parks the slot (role INACTIVE plus a retired flag), releases every
...
`dart_transport_topic_seqno` is the slot's next write seqno. It continues across retire
...
`DART_E_UNMATCHED_SEND`. `dart_topic_ready` shares the exact predicate. A variable
...
A sans-IO caller runs the exchange itself: `dart_transport_build_interest` and
`dart_transport_apply_peer_interest` for announces, the detail codec
(`dart_transport_detail_wants`, `dart_detail_req_build`, `dart_transport_detail_respond`,
`dart_transport_apply_peer_details`), and the paging codec (`dart_interest_req_build`,
`dart_interest_resp_head`, `dart_interest_resp_parse`, `dart_transport_interest_size`).
