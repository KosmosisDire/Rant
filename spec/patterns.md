...
- Vocabulary: DEFINITION is where the body or storage lives, REMOTE is a reference to one
  ...
  RESPONSE, the outcome is a `RantCallStatus`. No ROS vocabulary (RUNNING, never ACCEPTED,
...
- The layer depends on `node/runtime` only, through kind agnostic seams: create a topic
  ...
  `RANT_NO_PATTERNS` strips it.
- `RantTaskOpts` uses discrete zero default progress fields, not an embedded `RantQos`,
  because a zeroed `RantQos` means best effort and would contradict the reliable default.
...
A delivered message splits into `RantMsg.header` (the pattern prefix) and
`RantMsg.data` (the user payload the schema validates). A zero length payload on a prefix
...
- `@rsp`: `[u32 call_id][u8 status][u8 msg_len][msg]`. The message text is capped at
  `RANT_CALL_MSG_MAX` (255) and truncated, never refused. A non OK outcome with no text
...
so a stale token is `RANT_ERR_STATE`, never undefined behavior. `rant_function_progress`,
`rant_function_cancelled` and `rant_function_complete` are thread safe. Cancellation is
...
- A send only commits. A poll pass transmits. Retire and close drain the CANCELLED
  replies through `i_rant_node_flush_tx` (a transmit only drain) before tearing lanes down,
...
Task requests are always directed at one provider (`RantCallOpts.provider`, 0 = the
...
and remote sets get `RANT_ERR_ROLE`. Both channels keep the reliable history depth (10).
...
interest apply. It fires `RANT_E_DUPLICATE_AUTHORITY` once per (entity, peer) on both
...
callbacks, unlinks the handle and frees it. It is refused with `RANT_ERR_STATE` from
...
`t@prg` reliability is configurable per side with `RantTaskOpts.progress_best_effort`
...
`@rant/meta` rides this layer as one handle carrying both sides (mode 2 of the function
