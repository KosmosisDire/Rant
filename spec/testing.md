...
What each phase of `dart_test selftest` pins. The phase names match the comments in
`tests/dart_test.c`. Every phase runs on loopback on its own domain, offset from a per
...
- Open failure paths: a forced failure at each staged cleanup label returns NULL and
  ...
  `dart_last_error(NULL)` names MCAST_JOIN. An occupied data port collides, since the
  unicast bind takes no reuse, and names DART_E_BIND with the port and errno. An over
...
- Subset binding: a subscriber declaring a subset of the publisher's schema by name in
  ...
  refused on both sides with DART_SCHEMA_MISMATCH and a wrong size message is dropped.
...
- External interest: a publisher with about 300 topics cannot inline its interest, so the
  ...
  paging. No datagram ever exceeds DART_DGRAM_MAX, the match forms through the fetch, and
...
- HOSTILE (Windows): transport sends forced to would block. The first pass parks one
  ...
  surface as DART_EVICTED_UNSENT so every send is delivered or accounted.
...
- Match wait: a sample committed after our side matched but before the peer verified us
  ...
  drops at once but loudly with DART_E_UNMATCHED_SEND.
