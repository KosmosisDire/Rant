...
by default. Build with `RANT_NO_SHM` to strip it.
...
The SHM-DATA submessage is a DATA submessage with the `RANT_F_SHM` flag set.
...
- The build has SHM (no `RANT_NO_SHM`).
...
chunks of `RANT_SHM_CLASS_BASE << (k * RANT_SHM_CLASS_SHIFT)` bytes. The defaults are 64K,
...
The rest is a per node base with the pid folded in. The OS object name is `/rant.shm.`
...
re-send. A descriptor that stays unresolvable, such as mismatched `RANT_SHM_*`
constants between nodes, is skipped after `RANT_SHM_MAX_RETRY` tries with a
`RANT_MSG_LOST` event. The reader does not wedge.
...
|---------|---------|--------|
| RANT_SHM_CLASS_BASE | 64K | smallest chunk |
| RANT_SHM_CLASS_SHIFT | 2 | class growth ratio |
| RANT_SHM_N_CLASSES | 7 | number of classes |
| RANT_SHM_CHUNKS | 4 | chunks per segment when the node gives no keep_last |
| RANT_SHM_MAX_RETRY | 8 | resolve tries before skip |
